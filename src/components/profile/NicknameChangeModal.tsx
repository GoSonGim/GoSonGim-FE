...
export default function NicknameChangeModal({ isOpen, onClose, currentNickname }: NicknameChangeModalProps) {
  ...
  const handleChange = () => {
    if (newNickname.trim() && newNickname.length <= 8) {
  ...
  const isButtonDisabled = !newNickname.trim() || newNickname.length > 8 || updateNicknameMutation.isPending;
